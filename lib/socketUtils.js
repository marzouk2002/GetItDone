...
async function markAsRead(serverId, userId, toId,) {

    const server = await Server.findById(serverId)

    server.conversations
        .filter(conv => conv.between.includes(userId))
        .find(conv => conv.between.includes(toId))
    .viewed[userId] = true

    server.markModified("conversations");
    await server.save()

    const toReturn = await getConversations(serverId, userId)

    return toReturn
}


module.exports = { getContacts, getConversations, saveMsgToDb, markAsRead }